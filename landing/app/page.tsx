        ...
        <div className="flex-1 w-full max-w-sm md:max-w-none">
          <div className="rounded-xl border border-white/10 bg-zinc-900 p-5 font-mono text-xs">
            ...
            <div className="space-y-2 text-zinc-300">
              ...
              {[
                { name: 'Paracetamol 500mg', gst: '12%', total: '₹89.60' },
                { name: 'Vitamin D3 60K', gst: '5%', total: '₹126.00' },
                { name: 'BP Monitor Strip', gst: '18%', total: '₹236.00' },
              ].map((row) => (
                <div key={row.name} className="flex justify-between">
                  <span>{row.name}</span>
                  <span className="text-yellow-400">{row.gst}</span>
                  <span>{row.total}</span>
                </div>
              ))}
              <div className="border-t border-white/10 pt-3 mt-3 space-y-1">
                {[
                  { label: 'Taxable value', value: '₹393.72' },
                  { label: 'CGST', value: '₹28.94' },
                  { label: 'SGST', value: '₹28.94' },
                ].map((row) => (
  ...
  return (
    <section id="pricing" className="py-24 px-4 sm:px-6">
      <div className="mx-auto max-w-6xl text-center mb-12">
        <h2 className="text-3xl sm:text-4xl font-bold text-white">Honest pricing. No surprises.</h2>
